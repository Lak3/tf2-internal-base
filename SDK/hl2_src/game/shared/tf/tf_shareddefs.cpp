...
ETFGameType GetGameTypeFromName(const char* pszGameType)
{
	...
	{
		if (V_strcmp(pszGameType, s_aEnumGameTypeName[i]) == 0)
...
ETFFlagEventTypes GetCTFEventTypeFromName(const char* pszName)
{
	...
	{
		if (V_strcmp(pszName, GetCTFEventName((ETFFlagEventTypes)i)) == 0)
		{
...
RDScoreMethod_t GetRDScoreMethodFromName(const char* pszName)
{
	...
	{
		if (V_strcmp(pszName, GetRDScoreMethodName((RDScoreMethod_t)i)) == 0)
		{
...
ETFDmgCustom GetCustomDamageFromName(const char* pszCustomDmgName)
{
	...
	Error("Invalid Custom Damage Name");
...
{
	const char* const pszCurrentMap = I::EngineClient->GetLevelName();

	for (int i = 0; i < ARRAYSIZE(g_pszInvasionMaps); i++)
	{
		if (V_strcmp(g_pszInvasionMaps[i], pszCurrentMap) == 0)
			return true;
	}
