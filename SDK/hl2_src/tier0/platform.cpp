...
void Error(const char* fmt, ...)
{
	...
	vsprintf_s(cbuffer, fmt, va_alist);
	va_end(va_alist);

	MessageBoxA(HWND_DESKTOP, cbuffer, "FATAL ERROR", MB_ICONERROR);
}
...
void Error(const wchar_t* fmt, ...)
{
	...
	va_start(va_alist, fmt);
	vswprintf_s(wstr, fmt, va_alist);
	va_end(va_alist);

	MessageBoxW(HWND_DESKTOP, wstr, L"FATAL ERROR", MB_ICONERROR);
}
...
void Msg(const char* fmt, ...)
{
	...
	MessageBoxA(HWND_DESKTOP, cbuffer, "MESSAGE", MB_ICONINFORMATION);
}

void Msg(const wchar_t* fmt, ...)
{
	va_list va_alist;
	wchar_t wstr[1024] = { '\0' };
	...
	MessageBoxW(HWND_DESKTOP, wstr, L"MESSAGE", MB_ICONINFORMATION);
}
...
void Warning(const char* fmt, ...)
{
	...
	MessageBoxA(HWND_DESKTOP, cbuffer, "WARNING", MB_ICONINFORMATION);
}
...
void Warning(const wchar_t* fmt, ...)
{
	...
	MessageBoxW(HWND_DESKTOP, wstr, L"WARNING", MB_ICONINFORMATION);
}
...
void Plat_GetModuleFilename(char* pOut, int nMaxBytes)
{
	...
	GetModuleFileNameA(NULL, pOut, nMaxBytes);
...
bool vtune(bool resume)
{
	...
	if (!bInitialized)
	{
		...
		HINSTANCE pVTuneDLL = LoadLibraryA("vtuneapi.dll");
...
bool Is64BitOS()
{
	...
	static LPFN_ISWOW64PROCESS pfnIsWow64Process = (LPFN_ISWOW64PROCESS)GetProcAddress(GetModuleHandleA("kernel32"), "IsWow64Process");
