...
#include "../public/mathlib/ssequaternion.h"

bool s_bMathlibInitialized = false;

//I inlined this in the header
...
float QuaternionAngleDiff(const Quaternion& p, const Quaternion& q)
{
	...
	float angle = RAD2DEG(2.0f * asinf(sinang));
	return angle;
#else
	Quaternion q2;
	QuaternionAlign(p, q, q2);
...
float RangeCompressor(float flValue, float flMin, float flMax, float flBase)
{
	...
	float flTarget = flMid * 2.0f - 1.0f;

	if (fabsf(flTarget) > 0.75f)
	{
		...
		if (t < 1.0f)
		{
			if (flTarget > 0.0f)
			{
				flTarget = Hermite_Spline(0.75f, 1.0f, 0.75f, 0.0f, t);
			}
			else
			{
				flTarget = -Hermite_Spline(0.75f, 1.0f, 0.75f, 0.0f, t);
		...
		{
			flTarget = (flTarget > 0.0f) ? 1.0f : -1.0f;
		}
	}

	flMid = (flTarget + 1.0f) / 2.0f;
	flValue = flMin * (1.0f - flMid) + flMax * flMid;
	...
	float flFovX, float flFovY, Frustum_t& frustum)
{
	...
	float flTanX = tanf(DEG2RAD(flFovX));
	float flTanY = tanf(DEG2RAD(flFovY));
