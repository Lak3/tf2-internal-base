...
#include "../public/tier0/cpumonitoring.h"
...
static DWORD WINAPI HeartbeatThread(LPVOID)
{
	...
	std::this_thread::sleep_for(std::chrono::milliseconds(kFirstInterval));
	...
	for (;;)
	{
		...
		if (delay)
		{
			...
			{
				minActualFreq = Min(minActualFreq, s_frequencies[i]);
				maxActualFreq = Max(maxActualFreq, s_frequencies[i]);
			}
		...
		else
		{
			...
			std::this_thread::sleep_for(std::chrono::milliseconds(kMinimumDelay));
		}
	}
