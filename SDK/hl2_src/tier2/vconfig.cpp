...
#include "../public/tier2/vconfig.h"
...
bool GetVConfigRegistrySetting(const char* pName, char* pReturn, int size)
{
	...
	if (RegOpenKeyExA(HKEY_CURRENT_USER, "Environment", 0, KEY_QUERY_VALUE, &hregkey) != ERROR_SUCCESS)
		return false;

	// Get the value
	DWORD dwSize = size;
	if (RegQueryValueExA(hregkey, pName, NULL, NULL, (LPBYTE)pReturn, &dwSize) != ERROR_SUCCESS)
		return false;
...
void SetVConfigRegistrySetting(const char* pName, const char* pValue, bool bNotify)
{
	...
	RegSetValueExA(hregkey, pName, 0, nType, (const unsigned char*)pValue, (int)strlen(pValue));
...
bool RemoveObsoleteVConfigRegistrySetting(const char* pValueName, char* pOldValue, int size)
{
	// Open the key
	HKEY hregkey;
	if (RegOpenKeyExA(HKEY_CURRENT_USER, "Environment", 0, (REGSAM)KEY_ALL_ACCESS, &hregkey) != ERROR_SUCCESS)
		return false;
	...
	if (pOldValue != NULL)
	{
		...
		// Get the value
		if (RegQueryValueExA(hregkey, pValueName, NULL, NULL, (LPBYTE)pOldValue, &dwSize) != ERROR_SUCCESS)
			return false;
	}

	// Remove the value
	if (RegDeleteValueA(hregkey, pValueName) != ERROR_SUCCESS)
		return false;
