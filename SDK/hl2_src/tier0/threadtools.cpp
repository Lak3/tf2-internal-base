...
#include "../public/tier0/dynfunction.h"
...
#include "../public/tier0/valve_minmax_off.h"
#include <memory>
#include "../public/tier0/valve_minmax_on.h"

#include "../public/tier0/threadtools.h"
#include "../public/tier0/vcrmode.h"

#ifdef _X360
#include "xbox/xbox_win32stubs.h"
#endif

#include "../public/tier0/vprof_telemetry.h"
...
ThreadHandle_t CreateSimpleThread(ThreadFunc_t pfnThread, void* pParam, ThreadId_t* pID, unsigned stackSize)
{
	...
	unsigned int nThreadID = 0u;

	HANDLE h = VCRHook_CreateThread(NULL, stackSize, ThreadProcConvert, new ThreadProcInfo_t(pfnThread, pParam), CREATE_SUSPENDED, &nThreadID);
	
	*pID = static_cast<ThreadId_t>(nThreadID);
...
#ifdef THREADS_DEBUG
#ifdef _WIN32
	Warning("Thread synchronization object is unuseable");
#elif defined(POSIX)
	Warning(m_bInitalized, "Thread synchronization object is unuseable");
#endif
#endif
}
...
CThreadEvent::CThreadEvent(bool bManualReset)
{
	...
	Warning("Failed to create event (error 0x%x)", GetLastError());
#elif defined( POSIX )
...
CThreadEvent::CThreadEvent(HANDLE hHandle)
{
	...
	Warning("Null event passed into constructor");
}
#endif
...
CThreadSemaphore::CThreadSemaphore(long initialValue, long maxValue)
{
	if (maxValue)
	{
		...
		Warning("Failed to create semaphore (error 0x%x)", GetLastError());
	}
	else
	{
		m_hSyncObject = NULL;
...
{
	m_hSyncObject = CreateMutexA(NULL, bEstablishInitialOwnership, pszName);

	Warning("Failed to create mutex (error 0x%x)", GetLastError());
}
...
CThread::~CThread()
{
...
#endif
	{
		if (IsAlive())
		{
			...
			Error("Illegal termination of worker thread! Threads must negotiate an end to the thread before the CThread object is destroyed. (%s, %s)\n", __FILE__, __LINE__);
#endif
...
bool CThread::Start(unsigned nBytesStack)
{
	...
	if (IsAlive())
	{
		Warning("Tried to create a thread that has already been created!");
		return false;
	}
	...
	m_threadId = static_cast<ThreadId_t>(nThreadID);

	if (!hThread)
	{
		Error("Failed to create thread (error 0x%x)", GetLastError());
		return false;
	}
...
void CThread::Stop(int exitCode)
{
	...
	if (GetCurrentCThread() == this)
	{
		...
		Warning("Only thread can stop self: Use a higher-level protocol");
}
...
void CThread::SuspendCooperative()
{
	...
	else
	{
		...
		Warning(L"Suspend not called from worker thread, this would be a bug");
	}
}
...
bool CThread::WaitForCreateComplete(CThreadEvent* pEvent)
{
	...
	{
		Warning("Probably deadlock or failure waiting for thread to initialize.");
		return false;
	}
	return true;
}
...
int CWorkerThread::WaitForReply(unsigned timeout, WaitFunc_t pfnWait)
{
	...
	if (result != WAIT_OBJECT_0 + 1)
	{
		...
		{
			Msg("Thread failed to respond, probably exited\n");
			m_EventSend.Reset();
			m_ReturnVal = WTCR_TIMEOUT;
		}
		else
