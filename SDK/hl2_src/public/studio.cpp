...
mstudioanim_t* mstudioanimdesc_t::pAnimBlock(int block, int index) const
{
	...
	byte* pAnimBlock = nullptr; //pStudiohdr()->GetAnimBlock(block);
	if (pAnimBlock)
	{
...
mstudioikrule_t* mstudioanimdesc_t::pIKRule(int i) const
{
	...
	else if (animblockikruleindex)
	{
		...
		{
			byte* pAnimBlocks = nullptr; //pStudiohdr()->GetAnimBlock(animblock);
...
//Unfunctional, set up SetActivityForSequence and define CLIENT_DLL
void CStudioHdr::CActivityToSequenceMapping::Initialize(CStudioHdr* __restrict pstudiohdr)
{
	...
	if (m_pSequenceTuples == NULL)
		Warning("Tried to double-initialize sequence mapping for %s", pstudiohdr->pszName());
