...
public:
	...
	int			operator=(int i) { Set(NULL); return 0; }
	T* operator=(T* p) { Set(p); return p; }

	bool        operator !() const { return (!Get()); }
	bool        operator!=(int i) const { return (Get() != NULL); }
	bool        operator==(int i) const { return (Get() == NULL); }
...
#else
	HANDLE handles[64];
	for (int i = 0; i < Min(nEvents, 64); i++)
...
{
public:
	CThreadSpinRWLock() { memset(this, 0, sizeof(*this)); }
...
inline bool CThreadMutex::assertOwnedByCurrentThread()
{
	...
	if (ThreadGetCurrentId() == m_currentOwnerID)
		return true;
	Warning("Expected thread %u as owner of lock %p, but %u owns", ThreadGetCurrentId(), (CRITICAL_SECTION*)&m_CriticalSection, m_currentOwnerID);
	return false;
#else
	return true;
#endif
