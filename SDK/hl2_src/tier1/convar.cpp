...
#include "../public/tier1/convar_serverbounded.h" //Includes convar.h
...
void ConVar_Register(int nCVarFlag, IConCommandBaseAccessor* pAccessor)
{
	...
	I::Cvar->ProcessQueuedMaterialThreadConVarSets();
...
void ConVar_Unregister()
{
	...
	assert(s_nDLLIdentifier >= 0);
	I::Cvar->UnregisterConCommands(s_nDLLIdentifier);
	s_nDLLIdentifier = -1;
	s_bRegistered = false;
}
...
void ConCommand::Dispatch(const CCommand& command)
{
	...
	Error("Encountered ConCommand '%s' without a callback!\n", GetName());
}
...
void ConVar::InternalSetValue(const char* value)
{
	...
	{
		if (I::Cvar && !I::Cvar->IsMaterialThreadSetAllowed())
		{
			I::Cvar->QueueMaterialThreadSetValue(this, value);
			return;
		}
	}
...
void ConVar::ChangeStringValue(const char* tempVal, float flOldValue)
{
	...
	if (V_strcmp(pszOldValue, m_pszString) != 0)
	{
		...
		I::Cvar->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
	}
...
void ConVar::InternalSetFloatValue(float fNewValue, bool bForce /*= false */)
{
	...
	if (IsFlagSet(FCVAR_MATERIAL_THREAD_MASK))
	{
		if (I::Cvar && !I::Cvar->IsMaterialThreadSetAllowed())
		{
			I::Cvar->QueueMaterialThreadSetValue(this, fNewValue);
			return;
		}
	}
...
void ConVar::InternalSetIntValue(int nValue)
{
	...
	if (IsFlagSet(FCVAR_MATERIAL_THREAD_MASK))
	{
		if (I::Cvar && !I::Cvar->IsMaterialThreadSetAllowed())
		{
			I::Cvar->QueueMaterialThreadSetValue(this, nValue);
			return;
		}
	}
...
{
	m_pConVar = I::Cvar ? I::Cvar->FindVar(pName) : &s_EmptyConVar;
	if (!m_pConVar)
	{
...
void ConVar_PrintFlags(const ConCommandBase* var)
{
	...
	{
		I::Cvar->ConsolePrintf(" game");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_CLIENTDLL))
	{
		I::Cvar->ConsolePrintf(" client");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_ARCHIVE))
	{
		I::Cvar->ConsolePrintf(" archive");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_NOTIFY))
	{
		I::Cvar->ConsolePrintf(" notify");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_SPONLY))
	{
		I::Cvar->ConsolePrintf(" singleplayer");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_NOT_CONNECTED))
	{
		I::Cvar->ConsolePrintf(" notconnected");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_CHEAT))
	{
		I::Cvar->ConsolePrintf(" cheat");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_REPLICATED))
	{
		I::Cvar->ConsolePrintf(" replicated");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_SERVER_CAN_EXECUTE))
	{
		I::Cvar->ConsolePrintf(" server_can_execute");
		any = true;
	}

	if (var->IsFlagSet(FCVAR_CLIENTCMD_CAN_EXECUTE))
	{
		I::Cvar->ConsolePrintf(" clientcmd_can_execute");
		any = true;
	}
...
void ConVar_PrintDescription(const ConCommandBase* pVar)
{
	...
	if (!pVar->IsCommand())
	{
		...
		if (value)
		{
			I::Cvar->ConsoleColorPrintf(clr, "\"%s\" = \"%s\"", var->GetName(), value);

			if (stricmp(value, var->GetDefault()))
			{
				I::Cvar->ConsolePrintf(" ( def. \"%s\" )", var->GetDefault());
			}
		}

		if (bMin)
		{
			I::Cvar->ConsolePrintf(" min. %f", fMin);
		}
		if (bMax)
		{
			I::Cvar->ConsolePrintf(" max. %f", fMax);
		}
		...
		{
			I::Cvar->ConsoleColorPrintf(clr, "** NOTE: The real value is %.3f but the server has temporarily restricted it to %.3f **\n",
	...
	else
	{
		...
		I::Cvar->ConsoleColorPrintf(clr, "\"%s\"\n", var->GetName());
	}
