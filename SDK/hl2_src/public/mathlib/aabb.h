...
public:
	...
	{
		const float flXInterval = Max(0.0f, Max(vPoint.x - m_vMaxBounds.x, m_vMinBounds.x - vPoint.x));
		const float flYInterval = Max(0.0f, Max(vPoint.y - m_vMaxBounds.y, m_vMinBounds.y - vPoint.y));
		const float flZInterval = Max(0.0f, Max(vPoint.z - m_vMaxBounds.z, m_vMinBounds.z - vPoint.z));
	...
	{
		const Vector vDelta = (m_vMaxBounds - m_vMinBounds);
	...
	{
		const Vector vHalfExpand = (VectorMax(Vector(0, 0, 0), vMinSize - GetSize()) * 0.5f);
...
{
	const Vector vMin = VectorMin(aabb1.m_vMinBounds, aabb2.m_vMinBounds);
	const Vector vMax = VectorMax(aabb1.m_vMaxBounds, aabb2.m_vMaxBounds);
