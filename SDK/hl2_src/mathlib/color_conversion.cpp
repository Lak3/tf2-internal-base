...
void BuildGammaTable(float gamma, float texGamma, float brightness, int overbright)
{
	...
	for (i = 0; i < 256; i++)
	{
		inf = 255 * static_cast<int>(powf(i / 255.0f, g1));
	...
	for (i = 0; i < 1024; i++)
	{
		...
		inf = 255 * static_cast<int>(powf(f, g));
	...
	*/
	...
	for (i = 0; i < 256; i++)
	{
		...
		g_Mathlib_LinearToGamma[i] = LinearToGammaFullRange(i / 255.0f);

		// convert from sRGB gamma space (0..1) to linear space (0..1)
		g_Mathlib_GammaToLinear[i] = GammaToLinearFullRange(i / 255.0f);
	}

	for (i = 0; i < 1024; i++)
	{
		// convert from linear space (0..1) to nonlinear texture space (0..255)
		lineartotexture[i] = static_cast<int>(powf(i / 1023.0, 1.0 / texGamma)) * 255;
	}
...
float X360GammaToLinear(float fl360GammaValue)
{
	...
	if (fl360GammaValue < (96.0f / 255.0f))
	{
		...
		else
		{
			...
			flLinearValue += floorf(flLinearValue * (1.0f / 512.0f));
		}
	}
	else
	{
		if (fl360GammaValue < (192.0f / 255.0f))
		{
			...
			flLinearValue += floorf(flLinearValue * (1.0f / 256.0f));
		}
		else
		{
			flLinearValue = fl360GammaValue * (255.0f * 8.0f) - 1024.0f;
			flLinearValue += floorf(flLinearValue * (1.0f / 128.0f));
		}
	}
